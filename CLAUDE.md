...
- TOC (Table of Contents) sticky su tutte le pagine (introdotto su 3 pagine corso
  il 2026-06-04, esteso a tutte le pagine il 2026-06-05):
  ...
  **Struttura HTML** (aggiornata — label è flex sibling del track, NON dentro il track):
  ```
  <nav .pb-toc>
    <div .pb-toc__bar>       ← flex container; desktop: justify-center; mobile: stretch
      <span .pb-toc__label>  ← "Menù" — flex item a piena altezza su mobile
      <div .pb-toc__track>   ← flex: 1, overflow-x: auto (solo le pillole)
        <a .pb-toc__pill>…
    <div .pb-toc__progress>  ← barra rossa 2px che si riempie con lo scroll
  ```
  **Perché la label è fuori dal track**: `position: sticky` dentro `overflow-x: auto`
  causa gap sinistro, vibrazione durante lo scroll e altezza collassata. Con la label
  come flex sibling di `.pb-toc__bar` (che ha `align-items: stretch`), la label si allunga
  automaticamente all'altezza del bar senza alcun posizionamento manuale.

  **Mobile** (`≤768px`):
  - Label: `align-self: stretch` — piena altezza garantita dal flex parent
  - Nessun gradiente né fade: taglio netto (overflow del track) a sinistra e a destra
  - Track: `flex: 1; padding-left: 12px` — gap visivo dopo il bordo della label

  **Desktop**: `.pb-toc__bar` con `justify-content: center` centra il gruppo label+pillole.

  **Scroll spy** (`<script is:inline>`):
  - OFFSET dinamico da `tocEl.getBoundingClientRect().bottom + 10`
  - Default `best = -1`: nessuna pillola attiva sulla Hero
  - Centering pillola attiva: `track.scrollLeft = pillCenterFromNav - navWidth/2`
    (centra rispetto all'intera larghezza dello schermo, non solo del track)
  - `activePill.offsetLeft` misurato dall'`offsetParent` (nav), formula universale che
    funziona sia con label dentro che fuori dal track

  **Progress bar** (`.pb-toc__progress`): rispecchia lo scroll reale della pagina
  (`scrollY / max * 100`), senza forzature a 100% sull'ultima sezione.

  **CSS classi** in `src/styles/global.css` (`@layer components`):
  `.pb-toc`, `.pb-toc__bar`, `.pb-toc__track`, `.pb-toc__label`, `.pb-toc__pill` (+ stati),
  `.pb-toc__progress`, `.pb-toc__progress-fill`,
  `.pb-toc-section` (scroll-margin-top: 130px desktop / 118px mobile).

  **Pillole**: 12px, tracking-widest uppercase, semibold. Inattiva: `text-pb-text-secondary`.
  Attiva: `bg-brand border-brand text-white`. Hover attiva: `#A51818`. `isolation: isolate`
  previene artifact GPU. Transizione solo su `border-color` e `color` (background istantaneo).

  **Pagine con TOC** (tutte tranne 404):
  | Pagina | Pillole principali |
  |---|---|
  ...
  | `/contatti/` | Contatti / Scrivici / Dove siamo / Social / Orari di apertura |
  | `/orari/` | Orari dei corsi / Prezzi / Certificato Medico |
